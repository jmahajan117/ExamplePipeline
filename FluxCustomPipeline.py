...
class FluxCustomPipeline(
    ...
    def _encode_vae_image(self, image: torch.Tensor, generator: torch.Generator):
        if isinstance(generator, list):
            image_latents = [
                self.retrieve_latents(self.vae.encode(image[i : i + 1]), generator=generator[i])
                for i in range(image.shape[0])
            ]
            image_latents = torch.cat(image_latents, dim=0)
        else:
            image_latents = self.retrieve_latents(self.vae.encode(image), generator=generator)
    ...
    def __call__(
            ...
            for i, t in enumerate(timesteps):
                ...
                if collect_features:
                    for j in range(len(self.transformer.features)):
                        # print(self.transformer.features[j]['hidden'].size())
                        if feats is None:
        ...
        if not run_feats:
            latent_copy = self._unpack_latents(latent_copy, height, width, self.vae_scale_factor)
            latent_copy = (latent_copy / self.vae.config.scaling_factor) + self.vae.config.shift_factor
            image_agg = self.vae.decode(latent_copy, return_dict=False)[0]
            image_agg = self.image_processor.postprocess(image_agg, output_type="pt")
            image_agg = torch.nn.functional.interpolate(image_agg, size=32)

            self.outhead = self.agg_network(image_agg.to(self.feature_dtype).to(device))
